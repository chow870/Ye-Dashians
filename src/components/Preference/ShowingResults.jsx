...
export default function ShowingResults() {
    ...
    console.log(friendSuggestedIds)
    ...
    useEffect(() => {
        ...
        socket.on("private-message-recieved", (data) => {
            ...
            if (data.sentObj.suggestor != myId) {
                setFriendSuggestedIds((prev)=> new Set(prev).add(data.sentObj.suggestion))
            }
    ...
    const HandlerNavigateMoreDetailsPage = (item) => {
        const url = `/preference/moredetails?item=${encodeURIComponent(JSON.stringify(item))}&mylocation=${encodeURIComponent(JSON.stringify(mylocation))}&destinationLocation=${encodeURIComponent(JSON.stringify(item.geometry.location))}`;
  ...
  return (
    <div className="flex h-screen bg-black text-white overflow-hidden">
      {/* Left Side: Options */}
      <div className="flex flex-col w-3/5 p-5 gap-6 overflow-y-auto ">
      ...
      {/* Right Side: Chat */}
      <div className=" w-2/5 height-100% p-4 bg-gray-800 text-white rounded-lg flex flex-col gap-4 mt-4">
